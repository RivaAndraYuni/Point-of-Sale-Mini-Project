...
export default function Pembayaran() {
  ...
  const handleClosePaymentCompleteModal = () => {
  ...
  return (
    <div className="m-2 max-w-screen-2xl grid grid-cols-[6.7fr,3.3fr] text-lg">
        ...
        <div className="flex flex-col h-[470px] overflow-y-auto">
          {dataOrder.map((item) => (
            ...
            >
              ...
              <p>{item.qty}</p>
              <p>{toRupiah(item.price * item.qty)}</p>
            </div>
          ))}
        </div>
        <div className="my-5">
      ...
      <div className="border border-l-2">
        ...
        <div className="m-5">
          <div>Total: {toRupiah(totalHarga)}</div>
          <div>
            Dibayar:{" "}
            <input
              type="number"
          ...
          <div>
            Kembalian:{" "}
            {kembalian >= 0 ? toRupiah(kembalian) : "Tidak cukup uang"}
          </div>
      ...
      {showSuccessNotification && (
        <NotificationModal
          message="Pembayaran telah berhasil. Mohon tunggu sebentar saat kami menyiapkan pesanan Anda."
          onClose={handleClosePaymentCompleteModal}
        />
      )}
    </div>
  );
