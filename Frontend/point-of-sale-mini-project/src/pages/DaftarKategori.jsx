...
export default function DaftarKategori() {
  ...
  const [deletionNotAllowed, setDeletionNotAllowed] = useState(false);
  const [deletionSuccess, setDeletionSuccess] = useState(false);
  const [deletingConfirmation, setDeletingConfirmation] = useState(null);

  const handleDelete = async (id) => {
    try {
      if (categoryProductCounts[id] > 0 || id == 1) {
        setDeletionNotAllowed(true);
        return;
      }
  ...
  const handleCloseDeletionNotAllowed = () => {
    setDeletionNotAllowed(false);
  };
  ...
  return (
    <div className=" w-full bg-yellow-300 h-screen">
      ...
      {deletionNotAllowed && (
        <NotificationModal
          message="Kategori ini memiliki produk dan tidak dapat dihapus !"
          onClose={handleCloseDeletionNotAllowed}
        />
      )}
